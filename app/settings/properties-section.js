...
const {
  default: Observer,
} = require('@researchgate/react-intersection-observer');
  ...
  renderProperties(properties, prefix) {
    return Object.keys(properties).map((propertyName) => {
      ...
      if (schema.type === 'object') {
        ...
        const path = [prefix, propertyName].filter(partialPath => partialPath.length > 0).join('.');

        property = createElement(Property, {
          schema,
          key: path,
          property: path,
        });
